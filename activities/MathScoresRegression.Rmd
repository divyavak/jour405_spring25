...
ANSWER HERE
Here the red line representing linear regression shows that when FARMS rates increase, so do math scores. However, this theory isn't extremely consistent since most of the plot points do not fall around the line and are a bit scattered. 
...
ANSWER HERE
With the low p value and high residuals, we can identify that the model is not worth using since there might be other factors at hand influencing this relationship. 
...
ANSWER HERE
The residuals are very high for some schools like Whitemarsh where the expected was 60 and the actual was 26. 
...
ANSWER HERE
I think this is not the best conclusion to draw since the data does not strongly suggest a correlation between FARMS and standardized math test scores meaning that other factors like access to tutoring might be more indicative of FARMS rates. 