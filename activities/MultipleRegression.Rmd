...
ANSWER HERE
The model has a very low p value with the F statistic of 15.44. While it isn't perfect, the residual standard error is high and we can use it since it is helpful. 
...
ANSWER HERE
The factor that has the most impact on wins seems to be Points Scored because that is what gets them the win over the other team, on the other hand Yards Gained seems to be the least impactful.
...
This model is helpful because we can analyze the various factors that might go into understanding the teams chances of winning. 