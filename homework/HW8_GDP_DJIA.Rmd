...
```{r}
gdpdjia <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
gdpdjia |> summarize(correlation = cor(GDP, DJIA, method="pearson"))
```
...
The correlation coefficient between the DJIA and GDP is 0.95 which is close to 1 meaning there is a strong positive correlation between the Dow Jones Industrial Average and the Gross Domestic Product, so when the GDP is high so is the DJIA. 
