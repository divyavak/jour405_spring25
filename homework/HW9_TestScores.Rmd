...
Yes, the scores have improved since all of the scores are significantly higher than the previous mean so if the mean was taken from these new scores, the number would be higher. 
...
The mean is 3 points higher and the standard deviation is lower. Beyond this, we can see that scores are a lot more closer since the standard deviation is lower meaning that overall, students performed way better which could be due to the new programs implementation. 
...
my_fill_color <- "lavender" # Replace with a color name like "royalblue", "darkgreen", etc.
...
ggplot(aes(x = classroom, y = reading_score)) +
  ...
  labs(
    title = "Reading Scores Rise After New Program Implementation",
...
The p value tells us that it is not likely that it is random that the reading scores improved but does not necessarily mean that it was due to the program implementation. But overall, the average scores increased by 3 points. 
...
New Reading Program Implementation improves scores by 3 points. 
A recent study shows that reading scores improved three points after the district implemented a new reading program. It is extremely unlikely that the scores increased randomly. 
### Reflection Question 6:
What metrics or outcomes beyond test scores might be important to track for assessing reading performance?
It might be important to track if resources are being allocated beyond the reading program to help increase scores, like more books or technological tools. 
